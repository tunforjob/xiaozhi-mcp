...
class TelegramClient:
    ...
    async def send_message(self, contact_name: str, message: str) -> dict:
        ...
        if contact_name not in self.allowed_contacts:
            error_msg = (
                f"Contact '{contact_name}' is not in allowed list. Allowed contacts: {sorted(self.allowed_contacts)}"
            )
        ...
        async with self.app:
            ...
            # Send message
            try:
                await self.app.send_message(chat_id=found_contact.id, text=message)
                ...
                return {
                    ...
                    'contact': {
                        'id': found_contact.id,
                        'name': ' '.join(filter(None, [found_contact.first_name, found_contact.last_name])),
                        'username': found_contact.username,
                    },
                }
            except Exception as e:
                error_msg = f'Failed to send message: {e!s}'
                logger.error(error_msg)
                return {
                    'success': False,
                    'message': error_msg,
...
async def send_telegram_message(contact_name: str, message: str, config_path: str = 'telegram_config.json') -> dict:
    """
